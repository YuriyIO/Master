// NOTE: this file is a part of fast_double_parser project: https://github.com/lemire/fast_double_parser
// Licensed separately under Apache license 2.0, see https://github.com/lemire/fast_double_parser/blob/master/LICENSE
...
really_inline const char * parse_number(const char *p, double *outDouble) {
  ...
  if (found_minus) {
    ++p;
    negative = true;
    if (!is_integer(*p) && *p != '.') { // a negative sign must be followed by an integer
      return nullptr;
    }
  }
